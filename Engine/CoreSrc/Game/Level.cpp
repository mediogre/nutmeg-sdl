...
namespace Nutmeg {
	...
	void Level::update(float dt) {
		...
		for (int i=0; i<entities.count(); i++) {
			...
			if (cur_entity->isDestroyed() == true || cur_entity->update(dt) == false) {
				to_delete.append(&entities[i].getValue());
			}
		}

		for (int i=0; i<to_delete.count(); i++) {
			game->getConsole()->message("entity removed");
	...
	Entity *Level::createEntity(EntityNode *entity_node) {
		...
		Entity *entity = game->createEntity(entity_node->getEntityType());
	...
	void Level::removeEntity(Entity *entity) {
		...
		if (entity->getLevel() != this) return;
