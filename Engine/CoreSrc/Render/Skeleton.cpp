...
namespace Nutmeg {
	...
	void FramesSequence::playSound(float last_frame, float current_frame) const {
	...
	void AnimationState::AnimationLayer::update(float dt) {
		...
		if (current_source != NULL) {
			...
			current_source->getFramesSequence()->playSound(prev_frame, curr_frame);
		}
