...
namespace Nutmeg {
	...
	public:
		...
		void playSound(float last_frame, float current_frame) const;
	...
	public:
		...
		public:
			...
			AnimationLayer(const Skeleton *skeleton_, const AnimationSequence *sequence_, const BoneLayer *layer_) {
			...
			if (skeleton != NULL) {
				...
				if (sequence != NULL) {
					for (int i=0; i<sequence->getBoneLayersCount(); i++) {
						layers.append(new AnimationLayer(skeleton, sequence, &sequence->getBoneLayer(i)));
					}
				}

				if (layers.count() < 1) {
					layers.append(new AnimationLayer(skeleton, sequence, NULL));
				}
			}
		}
