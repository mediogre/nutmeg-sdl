...
namespace Nutmeg {
	...
	public:
		...
		virtual float getTextWidth(const char *text) {
			...
			return float(font->getTextWidth(text)) * k;
		}
	...
	*/
	...
	void RenderOpenGL1::drawSkinnedMesh(const RenderMesh &rmesh, const SolidArray <mat4> &matrices/*const Pose &pose*/) {

		SolidArray <vec3> pos;
		SolidArray <vec3> normal;
		//SolidArray <mat4> matrices;
		//pose.getMatrices(matrices);
		...
		if (animationQuality == 3) {
			for (int i=0; i<mesh.getVertexCount(); i++) {
				...
				for (int j=0; j<weightsCount; j++) {
					m = (mat4 *)&matrices[vert->bones.v[j]];
					w = vert->weights.v[j];
				...
				m = (mat4 *)&matrices[vert->bones.v[0]];
		...
		glVertexPointer(3, GL_FLOAT, sizeof(vec3), pos.data());
		glTexCoordPointer(2, GL_FLOAT, sizeof(Mesh::Vertex), &mesh.getVertex(0).texcoord);
		glNormalPointer(GL_FLOAT, sizeof(vec3), normal.data());
	...
	*/
	...
	//--------------------------------------------------------------------------
	*/
	void RenderOpenGL1::drawBillboard(const vec3 &pos, float size) {
	...
	void RenderOpenGL1::setAspectRatio(float ratio) {
		aspect_ratio = ratio;
	}
	...
	void RenderOpenGL1::drawText3d(const vec3 &pos, float cw_, float ch_, const char *text) {
