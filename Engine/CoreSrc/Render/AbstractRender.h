...
namespace Nutmeg {
	...
	public:
		...
		virtual void drawBillboard(const vec3 &pos, float size) = 0;
		virtual void drawText3d(const vec3 &pos, float cw, float ch, const char *text) = 0;
		virtual void renderRotateManipulator(const BSphere &m, const vec3 &x_color, const vec3 &y_color, const vec3 &z_color) = 0;
		virtual void renderSolidCircle(float radius) = 0;
		...
		virtual void drawSkinnedMesh(const RenderMesh &mesh, const SolidArray <mat4> &matrices/*const Pose &pose*/) = 0;
		...
		virtual void bindFont(const RenderFont &font);
	...
	public:
		...
		virtual int getTextWidth(const char *str) const;
